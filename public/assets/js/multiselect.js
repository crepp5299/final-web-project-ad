...
function UpdateIsValid(selectedPanel, isValid, isInvalid) {
  if (isValid) selectedPanel.classList.add('is-valid');
  else selectedPanel.classList.remove('is-valid');
  if (isInvalid) selectedPanel.classList.add('is-invalid');
  else selectedPanel.classList.remove('is-invalid');
}
...
(function(window, $) {
  ...
  var createPlugin = function createPlugin(element, settings, onDispose) {
    ...
    if (!configuration.adapter) {
      ...
      if (!stylingMethod) {
        if (useCss) stylingMethod = Bs4StylingMethodCss(configuration);
        else stylingMethod = Bs4StylingMethodJs(configuration);
      }
    ...
    if (configuration.optionsAdapter) optionsAdapter = configuration.optionsAdapter;
    else {
      ...
      if (configuration.options) {
        ...
        if (!configuration.createInputId)
          configuration.createInputId = function() {
        ...
        if (element.tagName == 'SELECT') selectElement = element;
        else {
        ...
        if (!configuration.getSize) {
          configuration.getSize = function() {
            ...
            else if (containerElement && containerElement.classList.contains('input-group-lg')) value = 'input-group-lg';
            else if (containerElement && containerElement.classList.contains('input-group-sm')) value = 'input-group-sm';
            return value;
          };
        }
        ...
        if (!configuration.createInputId)
          configuration.createInputId = function() {
            return ''
              .concat(configuration.containerClass, '-generated-input-')
              .concat((selectElement.id ? selectElement.id : selectElement.name).toLowerCase(), '-id');
          };
    ...
    if (!selectedItemContent) {
      ...
      if (!selectedItemContentStylingMethod) {
        if (useCss) selectedItemContentStylingMethod = Bs4SelectedItemContentStylingMethodCss(configuration, $);
        else selectedItemContentStylingMethod = Bs4SelectedItemContentStylingMethodJs(configuration, $);
      }
    ...
    if (!dropDownItemContent) {
      ...
      if (useCss) dropDownItemContentStylingMethod = Bs4DropDownItemContentStylingMethodCss(configuration, $);
      else dropDownItemContentStylingMethod = Bs4DropDownItemContentStylingMethodJs(configuration, $);
      dropDownItemContent = Bs4DropDownItemContent(dropDownItemContentStylingMethod, configuration, $);
    }

    var createStylingComposite = function createStylingComposite(
      container,
      selectedPanel,
    ...
    if (!placeholderText) {
      if (selectElement) placeholderText = $(selectElement).data('bsmultiselect-placeholder');
      else if (containerElement) placeholderText = $(containerElement).data('bsmultiselect-placeholder');
    }
